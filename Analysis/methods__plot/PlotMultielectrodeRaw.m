...
if ~isempty(utaharray_type) % assume utaharray is recorded using Cereplex
        ...
        %% plot raw data
        data_array_downsampled = decimate(data_array',factor_downsample)';
        ts_downsampled = decimate(ts,factor_downsample);
        figure; hold on;
...
if ~isempty(linearprobe_type) % assume linearprobe is recorded using Plexon        
        ...
        %% plot raw data
        data_probelfp_downsampled = decimate(data_probelfp',factor_downsample)';
        data_proberaw_downsampled = decimate(data_proberaw',factor_downsample)';
        ts_downsampled = decimate(ts,factor_downsample);
