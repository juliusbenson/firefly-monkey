...
phi = cellfun(@(ts,w) cumsum(w.*(ts>0),'omitnan')*prs.dt, {continuous.ts}, {continuous.w}, 'UniformOutput', false);
...
for i=1:ntrls
    ...
    %% fly position relative to monkey - screen    
    x = continuous(i).xfp_rel;
    y = continuous(i).yfp_rel; y(y < 0) = nan;
    [yle_targ,zle_targ,yre_targ,zre_targ] = world2eye(x,y,z,delta);
    continuous(i).z_targ = nanmean([zle_targ , zre_targ],2);
    continuous(i).y_targ = nanmean([yle_targ , yre_targ],2);
    %% target-tracking error
    continuous(i).tte_hor = continuous(i).y_targ - nanmean([continuous(i).yle , continuous(i).yre],2);
    continuous(i).tte_ver = continuous(i).z_targ - nanmean([continuous(i).zle , continuous(i).zre],2);
    continuous(i).tte_mag = sqrt(continuous(i).tte_hor.^2 + continuous(i).tte_ver.^2);
end
...
% target position
stats.tracking.targ_hor = {continuous.y_targ};
stats.tracking.targ_ver = {continuous.z_targ};
% target-tracking error
stats.tracking.tte_hor = {continuous.tte_hor};
stats.tracking.tte_ver = {continuous.tte_ver};
stats.tracking.tte_mag = {continuous.tte_mag};

%% trial type
if prs.basicFF || prs.twoFF
    max_dur = 7;
    stop_vel = 5;
elseif prs.visual_acc_control || prs.multisensory_acc_control
    max_dur = 8;
    stop_vel = 12;
end

if prs.unityVR
    goodtrls = ~((yf_monk<30) | (abs(v_monk)>stop_vel) | ([events.t_stop]>max_dur)); % remove trials in which monkey did not move at all or kept moving until the end
else
    goodtrls = ~((yf_monk<0) | (abs(v_monk)>stop_vel) | ([events.t_stop]>max_dur));
end
