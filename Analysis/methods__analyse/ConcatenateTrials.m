...
yt = cellfun(@(x,t,z) x(t>z(1) & t<z(2)),y2(:),t2(:),twin(:),'UniformOutput',false);

% concatenate stimulus
xt = [];
if ~isempty(x)
    x2 = cellfun(@(x) x(2:end-1),x,'UniformOutput',false);
%     x2 = cellfun(@(x) x(:),x2,'UniformOutput',false); % transpose is to reshape to column vector
    xt = cellfun(@(x,t,z) x(t>z(1) & t<z(2)),x2(:),t2(:),twin(:),'UniformOutput',false);
end
