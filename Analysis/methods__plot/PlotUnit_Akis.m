function PlotUnit_Akis(prs,behv,unit,plot_type,trial_type)
...
if isprop(unit,'trials'), spks_trials = unit.trials; 
elseif isfield(unit,'trials'), spks_trials = unit.trials; end
...
%             subplot(1,nconds,k); hold on; set(gcf,'Position',[100 100 400 400]);
            ...
            xlim([min(prs.ts.move) max(prs.ts.move)]); vline(0,'k'); xlabel('Time from movement onset (s)'); ylabel('Trial number');
        end
    case 'raster_targ'
...
%             subplot(1,nconds,k); hold on; set(gcf,'Position',[100 100 400 400]);
            ...
            xlim([min(prs.ts.target) max(prs.ts.target)]); vline(0,'k'); xlabel('Time from target onset (s)'); ylabel('Trial number');
        end
...
%             subplot(1,nconds,k); hold on; set(gcf,'Position',[300 300 400 400]);
            ...
            xlim([min(prs.ts.stop) max(prs.ts.stop)]); vline(0,'k'); xlabel('Time from stopping (s)'); ylabel('Trial number');
        end     
...
%             subplot(1,nconds,k); hold on; set(gcf,'Position',[100 100 400 400]);
            ...
            xlim([min(prs.ts.reward) max(prs.ts.reward)]); vline(0,'k'); xlabel('Time from stopping (s)'); ylabel('Trial number');
        end
    case 'raster_sac'
        %% raster plot - aligned to saccade onset
        for k=1:nconds
            ntrls = sum(trlindx(k,:));
            % select
            spks_selectedtrials_temp = spks_trials(trlindx(k,:));
            events_selectedtrials = {events_trials(trlindx(k,:)).t_sac};
            spks_selectedtrials = ShiftSpikes4saccades(spks_selectedtrials_temp,events_selectedtrials);
%             subplot(1,nconds,k); hold on; set(gcf,'Position',[100 100 400 400]);
            cnt = 0;
            for i=1:ntrls
                tspk = spks_selectedtrials(i).tspk;
                for n = 1:numel(events_selectedtrials{i})
                    if ~isempty(tspk), cnt = cnt+1; plot(tspk(1:30:end,n),cnt,'.r','MarkerSize',2,'Color',[.5 .5 .5]); end % plot every 30th spike
                end
            end
            xlim([min(prs.ts.saccade) max(prs.ts.saccade)]); vline(0,'k'); xlabel('Time from saccade (s)'); ylabel('Saccade number');
        end
...
%             hold on; set(gcf,'Position',[100 100 400 400]);
        ...
        legend(condnames);
        xlim([min(prs.ts.move) max(prs.ts.move)]); vline(0,'k'); xlabel('Time from movement onset (s)'); ylabel('Mean firing rate (spk/s)');
...
%             hold on; set(gcf,'Position',[100 100 400 400]);
        ...
        legend(condnames);
        xlim([min(prs.ts.target) max(prs.ts.target)]); vline(0,'k'); xlabel('Time from target onset (s)'); ylabel('Mean firing rate (spk/s)');
...
%             hold on; set(gcf,'Position',[100 100 400 400]);
        ...
        legend(condnames);
        xlim([min(prs.ts.stop) max(prs.ts.stop)]); vline(0,'k'); xlabel('Time from stopping (s)'); ylabel('Mean firing rate (spk/s)');
...
%             hold on; set(gcf,'Position',[100 100 400 400]);
        ...
        legend(condnames);
        xlim([min(prs.ts.reward) max(prs.ts.reward)]); vline(0,'k'); xlabel('Time from reward (s)'); ylabel('Mean firing rate (spk/s)');
    case 'rate_sac'
        %% mean firing rate - aligned to saccade onset
        for k=1:nconds
%             hold on; set(gcf,'Position',[100 100 400 400]);
            t = unit.stats.trialtype.(trial_type)(k).events.saccade.time;
            r = unit.stats.trialtype.(trial_type)(k).events.saccade.rate;
            plot(t,r);
        end
        legend(condnames);
        xlim([min(prs.ts.saccade) max(prs.ts.saccade)]); vline(0,'k'); xlabel('Time from saccade (s)'); ylabel('Mean firing rate (spk/s)');        
...
%                 xlabel(prs.GAM_varname{i}(1:min(4,length(prs.GAM_varname{i}))));
    ...
    case 'tuning_continuous'
        ...
        varnames = varnames(~ismember(varnames,{'sac_magdir','vheye','vhtarg','vhtte','rtheta','r','theta','a','alpha'}));
        nvars = numel(varnames);
        for k=1:nconds
            for i=1:nvars
                subplot(2,ceil(nvars/2),i); hold on;
                shadedErrorBar(continuous(k).(varnames{i}).tuning.stim.mu,continuous(k).(varnames{i}).tuning.rate.mu,...
                    continuous(k).(varnames{i}).tuning.rate.sem,'lineprops',{'Color',cmap(k,:)});
                xlabel(varnames{i}); ylabel('Firing rate [Hz]'); title(varnames{i});
            end
        end
