...
%% generate TTE across whole block to check distribution of values
if 0
trials_log = AddLOGData('m73s01.log',prs);

ch.xfp(:) =nan;
ch.yfp(:) =nan;
ch.phi = cumsum(ch.w,'omitnan')*dt;

for n = 1:numel(t.beg)
tindx = ts > t.beg(n) & ts < t.end(n);
ch.xfp(tindx) = trials_log(n).prs.xfp;
ch.yfp(tindx) = trials_log(n).prs.yfp;
ch.phi(tindx) = ch.phi(tindx) - ch.phi(tindx(find(tindx,1)));
end

ch.xfp = ch.xfp - ch.xmp;
ch.yfp = ch.yfp - ch.ymp;

R = @(phi) [cosd(phi) -sind(phi); sind(phi) cosd(phi)];
XY = cell2mat(arrayfun(@(phi,x,y) R(phi)*[x ; y], ch.phi, ch.xfp, ch.yfp, 'UniformOutput', false)');

ch.xfp = XY(1,:)'; ch.yfp = XY(2,:)';
...
ver_mean_targ = nanmean([zle_targ , zre_targ],2);
hor_mean_targ = nanmean([yle_targ , yre_targ],2);

tte.hor = hor_mean_targ - 0.5*(ch.yle+ch.yre);
tte.ver = ver_mean_targ - 0.5*(ch.zle+ch.zre);
tte.mag = sqrt(tte.hor.^2 + tte.ver.^2);

figure;
subplot(1,3,1); 
hist(tte.hor,100); title('horizontal'); xlabel('target-tracking error [deg]');
subplot(1,3,2); 
hist(tte.ver,100); title('vertical'); xlabel('target-tracking error [deg]');
subplot(1,3,3); 
hist(tte.mag,100); title('magnitude'); xlabel('target-tracking error [deg]');
suptitle('Spike2')
end
